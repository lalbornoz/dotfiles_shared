...
call roarie_commands#AddIMapping("&File", "exit_insert", "Exit insert mode", "Exit insert mode", '', '<C-Q>', '<Esc>')
if has('nvim')
call roarie_commands#AddTMapping("&File", "terminal_exit", "Exit terminal mode", "Exit terminal mode", '', '<Esc>', '<C-\><C-n>')
endif
...
call roarie_commands#AddSeparator("&File")
if has('nvim')
call roarie_commands#AddMapping("&File", "toggle_nvim_tree", "Toggle nvim-tree", "Toggle nvim-tree window", "<silent>", '<F12>', ':<C-U>NvimTreeToggle<CR>', "<fnalias>")
else
