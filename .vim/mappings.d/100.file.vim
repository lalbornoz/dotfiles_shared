...
call AddMapping("File", "Next buffer", "Go to next buffer in buffer list", "<silent>", '<S-Tab>', ':<C-U>bn<CR>')
call AddMapping("File", "Previous buffer", "Go to previous buffer in buffer list", "<silent>", '<C-S-Tab>', ':<C-U>bp<CR>')
...
call AddMapping("File", "Read from program...", "Read from program prompt into new scratch window", '', '<M-F9>', ':<C-U>CReadNewScratch ')
call AddMapping("File", "Read from program into buffer...", "Read from program prompt into current buffer", '', '<M-S-F9>', ':<C-U>r!')
call AddSeparator("File")
if has('nvim')
...
call AddMapping("File", "Telescope live_grep buffer...", "Telescope live_grep buffer...", "<silent>", '<M-F10>', ':lua require("telescope").extensions.menufacture.live_grep({path_display="hidden", search_dirs={"%:p"}})<CR>')
call AddMapping("File", "Toggle spectre", "Toggle spectre", "<silent>", '<C-F10>', '<Cmd>lua require("spectre").toggle()<CR>')
call AddMapping("File", "Spectre search word", "Spectre search word", "<silent>", '<C-S-F10>', '<Cmd>lua require("spectre").open_visual({select_word=true})<CR>')
call AddMapping("File", "Spectre search in file", "Spectre search in file", "<silent>", '<M-S-F10>', '<Cmd>lua require("spectre").open_file_search({select_word=true})<CR>')
else
call AddMapping("File", "Grep...", "Grep...", '', '<F10>', ':<C-U>Grep ')
call AddMapping("File", "Rrep...", "Recursive Grep...", '', '<M-F10>', ':<C-U>Rgrep ')
...
if has('nvim')
call AddMapping("File", "Toggle sidebar", "Toggle sidebar", "<silent>", '<C-F12>', ':lua require("sidebar-nvim").toggle({focus=true})<CR>')
...
call AddMapping("File", "Telescope buffers...", "Telescope buffers...", "<silent>", '<M-F12>', ':lua require("telescope.builtin").buffers()<CR>')
call AddMapping("File", "Telescope workspaces...", "Telescope workspaces...", "<silent>", '<C-S-F12>', ':Telescope workspaces<CR>')
endif
